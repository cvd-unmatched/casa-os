...
func (s *systemService) CheckForkUpdate() (needUpdate bool, current string, latest string, releaseNotes string) {
	...
	// in .github/workflows/release.yml) from the commits/PRs since the previous tag, so this
	// is always current with whatever actually shipped - nothing to maintain by hand.
	releaseNotes = gjson.Get(resp, "body").String()
