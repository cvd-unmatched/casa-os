...
func NewComposeAppFromYAML(yaml []byte, skipInterpolation, skipValidation bool) (*ComposeApp, error) {
	...
	// icon is no longer injected into each service's Docker labels here -
	// Compose treats labels as part of a service's config, so baking the
	// icon in made every icon change look like a real config change and
	// recreate the service just for a cosmetic edit. getContainerStats()
	// (container.go) now resolves each running container's icon by looking
	// up its compose project directly instead.
