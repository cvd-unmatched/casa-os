...
// loadIconByComposeProject maps each known compose project/app name to its
// stored icon, read straight from its compose file's x-casaos extension via
// the existing ComposeService.List, rather than from a label baked onto the
// service at save time (see NewComposeAppFromYAML - that injection used to
// exist for this exact lookup and had the side effect of making Compose
// recreate every service on any icon change, since it treats labels as part
// of a service's config).
func loadIconByComposeProject(ctx context.Context) map[string]string {
	result := map[string]string{}

	apps, err := NewComposeService().List(ctx)
	if err != nil {
		logger.Error("failed to list compose apps for icon lookup", zap.Error(err))
		return result
	}
...
func getContainerStats() {
	...
	iconByProject := loadIconByComposeProject(ctx)
	for i := 0; i < 100; i++ {
		if i%10 == 0 {
			containers, err = cli.ContainerList(context.Background(), types.ContainerListOptions{All: true})
			if err != nil {
		...
		for _, v := range containers {
			...
			go func(v types.Container, i int) {
				...
				// icon - prefer the compose project's stored icon (looked up
				// fresh each refresh, see loadIconByComposeProject) so that
				// changing an app's icon doesn't require baking it into the
				// service's own Docker labels, which would make Compose
				// think the service's config changed and recreate it just
				// for a cosmetic edit. Fall back to the legacy per-service
				// label for containers that still carry it (e.g. not yet
				// recreated since upgrading).
				if project, ok := v.Labels["com.docker.compose.project"]; ok {
					if icon, ok := iconByProject[project]; ok {
